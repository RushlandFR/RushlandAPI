...
public class PlayerInfo {
    ...
    public String generateXpBar() {
        ...
        Leveling nextLevel = Leveling.valueOf("LEVEL_" + next);
        int total = xp;
        double doublePercent = ((double) total / (double) nextLevel.getRequiredXp()) * 100.0;
    ...
    public void showPvpBoxRecap() {
        player.sendMessage("§9Points d'XP gagnés les 5 dernières minutes : §a" + sessionXp);
        if (level != 100) {
            player.sendMessage("§9Progression niveau " + level + " -> " + (level + 1) + ": " + generateXpBar());
        }
        final UUID finalUuid = uuid;
